...
class CityAgent(mg.GeoAgent):
    ...
    def __init__(self, model, geometry, crs="EPSG:4326", edge_id=None, edge_geom=None):
        ...
        self.move_speed = 0.05  # Move 5% of edge per step
    ...
    def _select_next_edge(self):
        """Select the next edge to walk along"""
        ...
        if self.current_edge_geom:
            ...
            # Find edges that start near this end point
            next_edges = self.model.find_connected_edges(end_point)
            
            if next_edges:
                # Pick a random connected edge
                next_edge_id, next_edge_geom = random.choice(next_edges)
...
class CityModel(mesa.Model):
    ...
    def __init__(self, num_agents=100):
        ...
        try:
            ...
            # Build network graph for efficient lookup
            self.edges = {}
            ...
            for _, row in edges_df.iterrows():
                ...
                # Map end points to edges (for finding connected edges)
                end_key = (round(end_point.x, 6), round(end_point.y, 6))
                self.node_to_edges[end_key].append((edge_id, edge_geom))
            
            print(f"[OK] Built network graph with {len(self.node_to_edges)} nodes")
    ...
    def find_connected_edges(self, point):
        """Find edges that start near the given point"""
